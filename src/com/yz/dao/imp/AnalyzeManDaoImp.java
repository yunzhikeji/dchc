...
@Component("analyzeManmanDao")
public class AnalyzeManDaoImp implements IAnalyzeManDao {
	 ...
	 */
	public boolean checkAnalyzeManExistsWithName(String queryString, Object[] p) {
	 ...
	 */
	public int updateAnalyzeManByhql(String queryString, Object[] p) {
		Session session=this.hibernateTemplate.getSessionFactory().getCurrentSession();
		Query query= session.createQuery(queryString);
		if(p!=null&&p.length>0){
			for (int i = 0; i < p.length; i++) {
	...
	public AnalyzeMan getAnalyzeManById(Integer upanalyzeManmanid) {
