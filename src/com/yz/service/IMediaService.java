package com.yz.service;

import java.util.List;

import com.yz.model.Media;
import com.yz.model.UserRole;

public interface IMediaService {

	//添加对象
	public abstract void add(Media media) throws Exception;

	//删除对象
	public abstract void delete(Media media);

	//删除某个id的对象
	public abstract void deleteById(int id);

	//修改对象
	public abstract void update(Media media);

	//获取所有对象
	public abstract List<Media> getMedias();

	//加载一个id的对象
	public abstract Media loadById(int id);

	//后台管理-页数获取
	public abstract int getPageCount(int totalCount, int size);

	//后台管理-获取总记录数
	public abstract int getTotalCount(int con, String convalue, UserRole user);

	//后台管理-获取符合条件的记录
	public abstract List<Media> queryList(int con, String convalue,UserRole user, int page,
			int size);


	public abstract Media getMediaById(Integer upmediaid);

	public abstract List<Media> loadByTypeAndPid(int i, Integer id);

	public abstract List<Media> loadInjurycaseByTypeAndPid(int mtype,
			Integer id);

	public abstract List<Media> loadClueByTypeAndPid(int currentJtype,
			Integer id);

	public abstract String setInjurycaseIdsSql(String queryString,String convalue, int mtype);

}